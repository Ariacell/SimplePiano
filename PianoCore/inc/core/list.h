...
public:
    List() = default;
    List(uint64_t size) : m_Buffer(size) {
    }
    List(const std::initializer_list<T>& list) : m_Buffer(list.size()) {
        for (auto& element : list)
            Add(element);
    }
    List(List&& other) : m_Buffer(other.m_Buffer.GetMaxCount()) {
        for (auto& val : other) {
    ...
    List(const List& other) : m_Buffer(other.m_Buffer.GetMaxCount()) {
        for (auto& val : other)
            Add(val);
    }
    ...
    T* At(int64_t idx) const {
        PIANOCORE_ASSERT(idx >= -(int64_t)m_Back || idx < (int64_t)Count());
    ...
    iterator begin() {
        return m_Buffer.Get() + m_Front;
    }
    iterator end() {
        return m_Buffer.Get() + m_Back;
    }
    ...
    public:
        ...
        TIteratorType* begin() {
            return m_Data + (int)m_Back - 1;
        }
        TIteratorType* end() {
            return m_Data + m_Front;
        }
        const TIteratorType* cbegin() const {
            return m_Data + (int)m_Back - 1;
        }
        const TIteratorType* cend() const {
            return m_Data + m_Front;
        }
        const TIteratorType* begin() const {
            return cbegin();
        }
        const TIteratorType* end() const {
            return cend();
        }

        TIteratorType& operator*() {
            return *m_Data;
        }
        const TIteratorType& operator*() const {
            return *m_Data;
        }

        reverse_iterator& operator++(
            int) {  // int parameter identifies as post-increment
            m_Data--;
            return *this;
        }
