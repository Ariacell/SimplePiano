...
// Helper: ImStableVector<>
// Allocating chunks of BLOCK_SIZE items. Objects pointers are never invalidated
...
enum ImGuiSelectableFlagsPrivate_ {
    ...
    ImGuiSelectableFlags_SelectOnNav =
        1 << 21,  // (WIP) Auto-select when moved into. This is not exposed in
                  ...
                  // need user to explicitly control focus scope. May be
                  // replaced with a BeginSelection() API.
...
struct IMGUI_API ImGuiInputTextState {
    ...
    const char* TextSrc;  // == TextA.Data unless read-only, in which case ==
                          // buf passed to InputText(). Field only set and valid
...
public:
    ...
    // [Obsolete] ImGuiWindow::CalcFontSize() was removed in 1.92.x because
...
IMGUI_API void RegisterUserTexture(
    ImTextureData* tex);  // Register external texture
...
// copy-and-pasted widgets which used FocusableItemRegister(): Refactored
// focus/nav/tabbing system in 1.82 and 1.84. If you have old/custom
...
typedef ImFontLoader
    ImFontBuilderIO;  // [renamed/changed in 1.92] The types are not actually
...
inline int ImFontAtlasRectId_GetGeneration(ImFontAtlasRectId id) {
    return (id & ImFontAtlasRectId_GenerationMask_) >>
...
struct ImFontAtlasRectEntry {
    int TargetIndex : 20;  // When Used: ImFontAtlasRectId -> into Rects[]. When
                           ...
                           // consume free-list.
    int Generation : 10;   // Increased each time the entry is reused for a new
                           // rectangle.
