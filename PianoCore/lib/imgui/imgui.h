...
// 'void*' to 'ImU64'. This allowed backends requirig 64-bit worth of data to
...
// The identifier is valid even before the texture has been uploaded to the
// GPU/graphics system. This is what gets passed to functions such as
// `ImGui::Image()`, `ImDrawList::AddImage()`. This is what gets stored in draw
// commands (`ImDrawCmd`) to identify a texture during rendering.
// - When a texture is created by user code (e.g. custom images), we directly
// stores the low-level ImTextureID.
...
//   happened.
// - There is no constructor to create a ImTextureID from a ImTextureData* as we
// don't expect this
//   to be useful to the end-user, and it would be erroneously called by many
//   legacy code.
...
enum ImGuiTreeNodeFlags_ {
    ...
    ImGuiTreeNodeFlags_NavLeftJumpsToParent =
        1 << 17,  // Nav: left arrow moves back to parent. This is processed in
                  // TreePop() when there's an unfullfilled Left nav request
                  // remaining.
...
struct ImGuiStyle {
    ...
    float FontScaleDpi;  // Additional global scale factor from viewport/monitor
                         // contents scale. When io.ConfigDpiScaleFonts is
                         // enabled, this is automatically overwritten when
...
struct ImDrawData {
    bool Valid;  // Only valid after Render() is called and before the next
                 // NewFrame() is called.
    int CmdListsCount;  // Number of ImDrawList* to render. (== CmdLists.Size).
                        // Exists for legacy reason.
    ...
    ImVector<ImTextureData*>*
        Textures;  // List of textures to update. Most of the times the list is
                   ...
                   // ImGui::GetPlatformIO().Textures[]. May be overriden or set
...
struct ImTextureData {
    //------------------------------------------ core / backend
    //---------------------------------------
    int UniqueID;  // w    -   // Sequential index to facilitate identifying a
                   // texture when debugging/printing. Unique per atlas.
    ...
    void SetTexID(ImTextureID tex_id) {
        TexID = tex_id;
    }  // Call after creating or destroying the texture. Never modify TexID
       // directly!
...
struct ImFontConfig {
    ...
    bool PixelSnapV;  // true     // Align Scaled GlyphOffset.y to pixel
                      // boundaries.
    ImS8
    ...
    // unsigned int  FontBuilderFlags;       // --       // [Renamed in 1.92] Ue
    // FontLoaderFlags.
...
struct ImFontAtlas {
    ...
    IMGUI_API ImFontAtlasRectId AddCustomRectFontGlyph(
        ...
        const ImVec2& offset = ImVec2(0,
                                      0));  // OBSOLETED in 1.92.X: Use custom
    ...
    // in 1.92.X: Renamed to FontLoaderFlags. int TexDesiredWidth;         //
    // OBSOLETED in 1.92.X: Force texture width before calling Build(). Must be
    ...
    // in 1.92.X typedef ImFontAtlasCustomRect     CustomRect;              //
...
// - Since 1.92.X a font may be rendered as any size! Therefore a font doesn't
