...
{ \
	if(!(condition)) { PIANOCORE_LOG_ERROR(message, ## __VA_ARGS__); } \
} \
...
#define PIANOCORE_ASSERT_NO_ARGS(condition) \
	PIANOCORE_INTERNAL_ASSERT_CHECK(condition, \
...
#define PIANOCORE_ASSERT(...) PIANOCORE_INTERNAL_ASSERT_GET_MACRO_NAME(__VA_ARGS__, PIANOCORE_ASSERT_ARGS, PIANOCORE_ASSERT_NO_ARGS)(__VA_ARGS__)